...
$query = "SELECT p.id_peli, p.titulo, p.descripcion, p.director, p.ano, p.imagen, p.likes, c.nombre AS categoria,
          ...
          WHERE 1=1"; // 1=1 es solo un truco para facilitar las condiciones adicionales
...
$stmt->bindParam(':usuarioId', $usuarioId, PDO::PARAM_INT); // Vinculamos el parámetro usuarioId

if ($categoria) {
    $stmt->bindParam(':categoria', $categoria, PDO::PARAM_STR); // Vinculamos el parámetro categoria
}
if ($director) {
    $stmt->bindParam(':director', $director, PDO::PARAM_STR); // Vinculamos el parámetro director
}
if ($ano) {
    $stmt->bindParam(':ano', $ano, PDO::PARAM_INT); // Vinculamos el parámetro ano
}
